...
- **Right pane** (`components/chat/chat-workspace.tsx`): a Kandungan / Cari / Tanya AI tab switcher. Kandungan (`components/toc/`) is a collapsible table of contents built from `data/index.json`; Cari (`components/search/`) is a BM25 search over `/api/search`; Tanya AI is the chat itself — Malay-first with EN toggle, `react-markdown` rendering, streaming with a stop button. Conversations persist to `localStorage` only (`lib/chat-store.ts`) — a "New chat" button and a grouped history dialog (Today / Past 7 days / Older, `chat-history-dialog.tsx`) replace a persistent sidebar; nothing is sent to a server.
